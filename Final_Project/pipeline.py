...
class SGI2VPipe(StableVideoDiffusionPipeline):
    ...
    def __call__(self, image, trajectory_points, height, width, num_frames, min_guidance_scale=1.0, max_guidance_scale=3.0,
        ...
        # print("image_latents's shape: ", image_latents.shape)   # debug line
        ...
        # print("added_time_ids:", added_time_ids.shape)  # debug line
        if do_classifier_free_guidance:
            added_time_ids = added_time_ids.repeat(1, 2)
            # print("duplicated added_time_ids:", added_time_ids.shape)   # debug line
        added_time_ids = added_time_ids.to("cuda").long()
        ...
        with self.progress_bar(total=self.dit.num_inference_steps) as progress_bar:     # set progress bar to visualize progress
            for i, t in enumerate(timesteps):
                # print("latent's shape: ", latents.shape)      # debug line
                # print("image_latents's shape:", image_latents.shape)      # debug line
                if do_classifier_free_guidance:
                ...
                # print("latent_model_input: ", latent_model_input.shape) #   debug line
                # print("image_latents_for_model: ", image_latents_for_model.shape)   #   debug line
                ...
                noise_pred = noise_pred[:, :, :4, :, :]     # only extract the first third dimension
                
                if guidance_scale is not None and guidance_scale.shape[0] > 1:
                ...
                if i == num_timesteps - 1 or ((i + 1) > num_warm_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()
